"""Sanitized read-only project listing with mocked GraphQL responses."""
...
SOURCE_REF_FIELDS = ["id", "stage", "owner.id", "owner.name", "created_at", "updated_at"]
...
def _extract_page_records(response: Mapping[str, Any]) -> list[Mapping[str, Any]]:
    connection = _list_project_connection(response)
...
def _list_project_connection(response: Mapping[str, Any]) -> Mapping[str, Any]:
...
def _normalize_project(raw_record: Mapping[str, Any]) -> tuple[dict[str, object], dict[str, object]]:
    source_id = _safe_string(raw_record.get("id"))
    source_ref_id = f"src-{source_id}"
    owner = _extract_owner(raw_record)
    record = {
        "id": source_id,
        "stage": _safe_string(raw_record.get("stage")),
        "owner": owner,
...
def _extract_owner(raw_record: Mapping[str, Any]) -> dict[str, str]:
    claim_by = raw_record.get("claimBy")
