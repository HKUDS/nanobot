...
| `nanobot provider login anthropic-oauth` | Store a Claude Code OAuth token generated by `claude setup-token` |
...
| `nanobot provider logout anthropic-oauth` | Remove nanobot's stored Anthropic OAuth token |
