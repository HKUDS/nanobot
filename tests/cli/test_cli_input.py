...
from janniebot.cli import commands
from janniebot.cli import stream as stream_mod
...
def mock_prompt_session():
    ...
    with patch("janniebot.cli.commands._PROMPT_SESSION", mock_session), \
         patch("janniebot.cli.commands.patch_stdout"):
        yield mock_session
...
def test_init_prompt_session_creates_session():
    ...
    with patch("janniebot.cli.commands.PromptSession") as mock_session, \
         patch("janniebot.cli.commands.FileHistory") as _mock_file_history, \
...
async def test_print_interactive_progress_line_pauses_spinner_before_printing():
    ...
    with patch("janniebot.cli.commands._print_interactive_line", side_effect=fake_print):
        thinking = stream_mod.ThinkingSpinner(console=mock_console)
        with thinking:
