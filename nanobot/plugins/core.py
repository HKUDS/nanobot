...
class TelegramUIPlugin:
    ...
    def _render_view(self, job, view_state: str) -> tuple[str, list]:
        ...
        if view_state.startswith("details"):
            ...
            return self._format_timeline_text(job), self._create_timeline_keyboard(job.id, job)
        else:
            return self._format_timeline_text(job), self._create_timeline_keyboard(job.id, job)

    def _format_timeline_text(self, job) -> str:
        """Format timeline view with job ID and emojis."""
        return f"{job.get_timeline_emoji(20)}\n\n🆔 `{job.id[:8]}`"
