...
def test_settings_hot_apply_updates_context_limits_and_tools(tmp_path):
    from nanobot.agent.tools.cron import CronTool
    from nanobot.cron.service import CronService

    cron = CronService(tmp_path / "cron.json")
    loop = AgentLoop(bus=MessageBus(), provider=_provider("test"), workspace=tmp_path,
                     model="test", cron_service=cron)
    config = Config()
    config.agents.defaults.timezone_mode = "manual"
    config.agents.defaults.timezone = "Asia/Shanghai"
    config.agents.defaults.max_tool_iterations = 42
    ...
    assert loop.subagents.max_concurrent_subagents == 7
    assert not loop.tools.has("exec")
    tool = loop.tools.get("cron")
    assert isinstance(tool, CronTool)
    assert "Asia/Shanghai" in tool.description
    config.tools.exec.enable = True
    loop.apply_settings(config)
    assert loop.tools.has("exec")


def test_workspace_switch_keeps_namespaces_and_updates_dependencies(tmp_path):
    first, second = tmp_path / "first", tmp_path / "second"
    first.mkdir()
    second.mkdir()
    loop = AgentLoop(bus=MessageBus(), provider=_provider("test"), workspace=first, model="test")
    session = loop.sessions.get_or_create("cli:first")
    session.add_message("user", "keep in first workspace")
    loop.sessions.save(session)
    original_dir = loop.sessions.sessions_dir
    loop.switch_workspace(second)
    assert loop.workspace == second
    assert loop.context.memory.workspace == second
    assert loop.subagents.workspace == second
    assert loop.sessions.sessions_dir != original_dir
    assert loop.sessions.get_cached("cli:first") is None
    loop.switch_workspace(first)
    assert loop.sessions.get_or_create("cli:first").messages[0]["content"] == "keep in first workspace"


@pytest.mark.asyncio
async def test_system_timezone_reschedules_cron_without_changing_user_timezone(tmp_path):
    from nanobot.cron.service import CronService
    from nanobot.cron.types import CronJob, CronPayload, CronSchedule

    cron = CronService(tmp_path / "jobs.json")
    cron.register_system_job(CronJob(id="dream", name="dream",
        schedule=CronSchedule(kind="cron", expr="0 9 * * *", tz="UTC"),
        payload=CronPayload(kind="system_event")))
    cron.register_system_job(CronJob(id="heartbeat", name="heartbeat",
        schedule=CronSchedule(kind="every", every_ms=3600000, tz="UTC"),
        payload=CronPayload(kind="system_event")))
    cron.add_job(name="user", schedule=CronSchedule(kind="cron", expr="0 9 * * *", tz="Europe/London"),
                 message="user schedule", channel="cli", to="user", session_key="cli:user")
    before = cron.get_job("dream").state.next_run_at_ms
    heartbeat_next = cron.get_job("heartbeat").state.next_run_at_ms
    cron.set_system_timezone("Asia/Shanghai")
    assert cron.get_job("dream").schedule.tz == "Asia/Shanghai"
