# Agent Cognitive Core Redesign — Detailed Design Document

> Date: 2026-03-27
> Author: Claude Opus 4.6 (collaborative session with project owner)
> Companion to: `specs/2026-03-27-agent-cognitive-redesign.md` (spec)
> Companion to: `reports/2026-03-27-agent-cognitive-redesign-report.md` (analysis report)

This document contains the FULL design detail as presented section by section during
the design review session. The spec is the authoritative reference; this document
preserves the complete reasoning, code examples, and discussion that informed it.

---

## Table of Contents

1. [Section 1: Architecture Overview & Core Patterns](#section-1)
2. [Section 2: The Cognitive Loop (TurnRunner)](#section-2)
3. [Section 3: The Guardrail Layer](#section-3)
4. [Section 4: The Prompt Layer (Context Architecture)](#section-4)
5. [Section 5: The Memory Interface (Three-Tier Model)](#section-5)
6. [Section 6: Entry Layer & Composition](#section-6)
7. [Section 7: Structural Patterns for Long-Term Stability](#section-7)
8. [Section 8: Migration Path](#section-8)
9. [Section 9: Testing Strategy](#section-9)
10. [Appendix A: Approach Selection (3 Approaches Evaluated)](#appendix-a)
11. [Appendix B: Cognitive Improvement — How the Three Layers Work Together](#appendix-b)
12. [Appendix C: Parallelization Assessment](#appendix-c)
...
│  (MessageProcessor — simplified)                             │
└──────────────────────────┬──────────────────────────────────┘
                           │
┌──────────────────────────▼──────────────────────────────────┐
│                    COGNITIVE LOOP                             │
│  Simple tool-use loop with guardrail checkpoints             │
│  (TurnRunner — new, replaces TurnOrchestrator)               │
└──────────────────────────┬──────────────────────────────────┘
                           │
┌──────────────────────────▼──────────────────────────────────┐
│                   GUARDRAIL LAYER                             │
...
change. This also means the loop rarely needs modification — it's the most stable component.
...
*Rationale:* The current ReflectPhase accumulated 7 different nudge types in one class
...
*Rationale:* The current context builder mixes concerns (role prompts, bootstrap files,
feedback, contacts, unavailable tools) and changes for many reasons. Layered composition
means each subsystem manages its own context contribution independently.

*Enforcement:* Each context layer is a function: `(workspace, config, query) → str | None`.
The builder calls each in order and joins non-None results. Adding a new layer means adding
a function, not modifying the builder.
...
*Rationale:* The current system saves facts but not strategies. This is why the agent
repeated the same failing approach across 5 sessions. Procedural memory breaks the
...
This is the core engine — the component that replaces `TurnOrchestrator`. The name changes
from "orchestrator" to "runner" to reflect its role: it **runs** the loop mechanically,
it doesn't **orchestrate** intelligence.
...
### What Changed vs Current TurnOrchestrator

| Concern | Current (TurnOrchestrator) | New (TurnRunner) |
|---------|---------------------------|-------------------|
| Planning | `_needs_planning()` keyword heuristic + prompt injection | Removed from loop. Reasoning protocol in system prompt handles this. |
| Plan enforcement | Detects "jumped to tools without plan", injects nudge | Removed. The prompt instructs reasoning; if model ignores it, guardrails catch the result. |
| Tool execution | Delegated to ActPhase class | Inline. Tool execution is 15 lines — it doesn't need its own class. |
| Result processing | Split across ActPhase and ReflectPhase | Inline result processing + guardrail checkpoint. |
| Reflection | ReflectPhase class with delegation advisor | Removed as a phase. Guardrail chain replaces it. Each concern (failure recovery, empty results, strategy loops) is an independent guardrail. |
| Delegation nudges | 7 different nudge types in ReflectPhase | Removed. With single-agent design, delegation nudges don't apply. If spawning is needed, the prompt instructs it. |
| Verification | Separate `AnswerVerifier` class, separate LLM call | Configurable. Default: prompt-only self-check. Optional: structured self-check as post-loop step (same context, one extra call). |
| Role switching | `active_*` fields, `effective_*` resolution | Removed entirely. Single agent, single model. |

### Working Memory (Per-Turn State)

The current `TurnState` has 17 fields, many for delegation and role switching. The new
state tracks what matters for reasoning:
...
class TurnState:
    ...
    tracker: ToolCallTracker          # existing failure tracking

    # Working memory (NEW — enables guardrails to reason about history)
    tool_results_log: list[ToolAttempt]   # what was tried + outcome
    current_strategy: str | None = None   # what approach the agent is using
...
The new `ToolAttempt` captures what guardrails need to detect patterns:
...
Key difference from current Verifier: **same context** (no information loss), **no
separate critique+revision dance** (one pass), **no confidence scoring** (the model
just fixes or passes through).
...
**Total: ~430 LOC** (down from ~970 for orchestrator + phases + verifier)
...
This is the extension point where reasoning enforcement lives — the part that catches
...
agent got an empty result AND it's repeating a strategy, the most specific guardrail
(empty result) should fire, not both. Priority ordering handles this.

### The Guardrails

Each guardrail is a small, focused module. Here are the initial set — the ones that
directly address the failures observed in the DS10540 case:

#### Guardrail 1: Empty Result Recovery

The guardrail that would have saved the DS10540 case.
...
class EmptyResultRecovery:
    ...
    def check(self, state, latest_results):
        ...
        # Check if this is a repeat of a previous empty result with similar args
...
class SkillTunnelVision:
    ...
    def check(self, state, latest_results):
        ...
        # Check if all tool calls have been exec (skill commands) with failures/empties
        recent = state.tool_results_log[-6:]  # last ~2 iterations
...
The existing failure budget concept, reimplemented as a guardrail.
...
class NoProgressBudget:
    ...
    def check(self, state, latest_results):
        ...
        # Count iterations that produced useful data
        useful = sum(
        ...
        if useful == 0 and state.iteration >= 4:
            return Intervention(
                ...
                severity="override",
                strategy_tag=None,  # not a learnable strategy
            )
        return None
```
...
This is the key maintenance property: **guardrails are additive**. You never modify
existing code to handle new patterns.
...
| File | LOC Target | Content |
|------|-----------|---------|
| `turn_guardrails.py` | ~150 | `Guardrail` protocol, `Intervention`, `GuardrailChain`, 5 guardrail classes |

All guardrails in one file while total is under 150 LOC. When the file grows past 300,
split into `guardrails/` subpackage with one file per guardrail — but not before.
...
The current system prompt assembles sections in an ad-hoc order with mixed concerns.
The new structure has a strict composition order where each section has one owner and
one purpose:
...
├─────────────────────────────────────────────────────────┤
│ 2. REASONING PROTOCOL  How to think before acting (NEW)  │
│    Owner: reasoning.md (static template)                 │
├─────────────────────────────────────────────────────────┤
│ 3. TOOL GUIDE         How to choose tools (NEW)          │
│    Owner: tool_guide.md (static template + dynamic list) │
├─────────────────────────────────────────────────────────┤
│ 4. STRATEGIES         Procedural memory (NEW)            │
...
class ContextContributor(Protocol):
    ...
    async def contribute(
        ...
        *,
        workspace: Path,
        config: AgentConfig,
        query: str,          # current user message (for memory retrieval)
    ) -> str | None:
...
**Why this pattern matters:** The current `build_system_prompt()` is a 100+ line method
that directly assembles every section. Adding a new section means modifying that method.
With contributors, adding a section means adding a class and registering it — the builder
never changes.
...
#### reasoning.md (NEW — the core cognitive improvement)
...
#### tool_guide.md (NEW — purpose-driven tool selection)
...
#### self_check.md (replaces verification_required.md)
...
Skills consumed by the agent should follow a structured pattern. This isn't a code
change — it's a content pattern for skill authors (including LLM-generated skills):
...
**Key change:** Decision logic comes first, reference material comes second. The agent
reads top-down — the first thing it sees should be HOW TO CHOOSE, not every possible
command.

### Tool Descriptions (Registry Level)

Tool descriptions in the registry get purpose and anti-pattern guidance. This is a data
change, not a code change — modify the `description` field when registering tools:

```python
# Current
ExecTool(description="Execute a shell command")
ListDirTool(description="List directory contents")
ReadFileTool(description="Read a file's contents")
...
which is what Claude Code's tool descriptions do. The model doesn't just know what a tool
does — it knows what it **shouldn't** be used for.

### Context Budget Management

The composition order matters for token pressure. When compression is needed, sections
are preserved by priority:
...
This means under token pressure, the agent loses workspace customization and feedback
history first — but never loses its reasoning protocol or tool selection guidance.

### File Targets

| File | LOC Target | Content |
|------|-----------|---------|
| `context/context.py` | ~200 | `ContextBuilder` + `ContextContributor` protocol + composition |
| `context/contributors/` | ~50 each | One file per contributor |
| `templates/prompts/reasoning.md` | ~40 lines | Reasoning protocol |
| `templates/prompts/tool_guide.md` | ~25 lines | Tool selection guide |
| `templates/prompts/self_check.md` | ~10 lines | Self-verification instructions |

**Net change to context/:** The current `context.py` (355 LOC) has a monolithic
`build_system_prompt()`. It becomes a thin composer (~200 LOC) with section logic moved
to contributor modules. Total LOC is similar but **each piece is independently modifiable
and testable**.

---

<a id="section-5"></a>
## Section 5: The Memory Interface (Three-Tier Model)
...
### Tier 2: Procedural Memory — Detailed Design
    ...
    async def extract_from_turn(
        ...
        for activation in guardrail_log:
            if activation.strategy_tag is None:
                continue
            # Check if the intervention led to success
            subsequent = [
...
```python
async def _build_strategy(self, activation, subsequent_successes, user_text):
    prompt = (
        "A tool-use strategy was discovered during this conversation.\n\n"
        f"User task: {user_text[:200]}\n"
        f"Failed approach: {activation.failed_tool} with {activation.failed_args}\n"
        f"Recovery: {activation.intervention_message[:200]}\n"
        f"Successful approach: {subsequent_successes[0].tool_name} "
        f"with {subsequent_successes[0].arguments}\n\n"
        "Write a concise, reusable strategy (2-3 sentences) that future "
        "sessions can use to avoid this failure. Include:\n"
        "1. What doesn't work and why\n"
        "2. What to do instead\n"
        "Format: domain|task_type|strategy|context"
    )
...
class ProceduralMemoryContributor(ContextContributor):
    ...
    async def contribute(self, *, workspace, config, query):
        strategies = await self._memory.retrieve_strategies(
            query=query,
            limit=5,              # max 5 strategies per turn
            min_confidence=0.3,   # include tentative strategies
        )
        if not strategies:
            return None
...
| File | LOC Target | Content |
|------|-----------|---------|
| `memory/strategy.py` | ~100 | `Strategy` model, `StrategyStore` (CRUD on strategies table) |
| `memory/strategy_extractor.py` | ~120 | `StrategyExtractor` (from guardrail recoveries + user feedback) |
| `context/contributors/procedural_memory.py` | ~50 | `ProceduralMemoryContributor` |
| DB migration | ~15 | `CREATE TABLE strategies` + indexes |
...
The current MessageProcessor (586 LOC) owns routing, session management, slash commands,
memory pre-checks, context assembly, orchestration, micro-extraction, and response
assembly. With routing removed, it simplifies to a pipeline coordinator:
...
| Concern | LOC removed |
|---------|------------|
| Routing pipeline (classify, resolve role) | ~30 |
| Role manager wiring (set_role_manager, apply, reset) | ~40 |
| Active settings management (set_active_settings, active_* fields) | ~30 |
| Classification result tracking | ~15 |
| Recovery attempt (separate AnswerVerifier call) | ~30 |
| **Total** | **~145** |

**Target: ~400 LOC** (down from 586)
...
async def build_agent(config, provider, bus, ...) -> AgentLoop:
    ...
    # ── New: strategy extractor ──────────────────────────
    strategy_extractor = StrategyExtractor(provider, memory)

    # ── New: TurnRunner (replaces TurnOrchestrator) ──────
    turn_runner = TurnRunner(
        ...
        context=context,
        config=config,
        provider=provider,      # for self-check and compression
    )

    # ── Simplified: processor (no router, no role manager)
    processor = MessageProcessor(
        turn_runner=turn_runner,
        context=context,
        sessions=sessions,
        memory_services=MemoryServices(store=memory, strategy_extractor=strategy_extractor),
        bus=bus,
        config=config,
        workspace=workspace,
    )
...
<a id="section-7"></a>
## Section 7: Structural Patterns for Long-Term Stability
...
**Applied to the new architecture:**
...
**Anti-pattern from the current codebase:** `turn_phases.py` changes when tool execution
logic changes OR when delegation nudge logic changes OR when planning heuristics change.
Three independent reasons to change in one file.
...
point — which means they're where complexity will accumulate. A limit of 10 prevents
...
self._dispatcher.active_messages = state.messages  # current code does this

# Post-construction wiring
loop._role_manager = TurnRoleManager(loop)  # current code does this

# Reaching into another component's internals
self._dispatcher.delegation_count = 0  # current code does this
```
...
```
Phase 1: Delete Dead Code              (risk: low,  value: high)
Phase 2: Introduce New Components       (risk: low,  value: high)
Phase 3: Rewire the Loop               (risk: medium, value: high)
Phase 4: Prompt Architecture            (risk: low,  value: highest)
Phase 5: Procedural Memory             (risk: low,  value: medium)
Phase 6: Cleanup & Documentation        (risk: low,  value: medium)
```
...
| delegation_advisor.py | **DELETE** | Nudge logic removed |
| delegation.py | **KEEP** | Sub-agent spawning |
...
**Phase 1: Delete Dead Code** (~1,600 LOC)
- Delete routing infrastructure (5 source files, 12 prompt files, 9 test files)
- Clean callers (factory, processor, orchestrator, turn_types, components, loop)
- Update manifests and boundary checks
- Detailed plan: `plans/2026-03-27-phase1-delete-dead-code.md`

**Phase 2: Introduce New Components** (standalone, no wiring)
- `turn_guardrails.py` with 5 guardrails + tests
- `ToolAttempt` dataclass in turn_types.py
- Prompt templates: reasoning.md, tool_guide.md, self_check.md
- `memory/strategy.py` with Strategy model + StrategyStore + tests

**Phase 3: Rewire the Loop** (feature-flag cutover)
- Create TurnRunner alongside TurnOrchestrator (same Orchestrator protocol)
- Feature-flag in factory: `agent.use_turn_runner` (default: false)
- Test with ScriptedProvider integration tests
- Cut over: make default, delete old orchestrator + phases + verifier

**Phase 4: Prompt Architecture** (parallelizable with Phase 3)
- Create ContextContributor protocol and 11 contributor classes
...
- Activate ProceduralMemoryContributor
...
**New: ~925 LOC** (18 files)
**Deleted: ~2,413 LOC** (26+ files)
**Modified: net -661 LOC** (6 files)
...
**Verdict:** Rejected — model-agnostic requirement means we can't rely on prompt-only.
...
<a id="appendix-b"></a>
## Appendix B: Cognitive Improvement — How the Three Layers Work Together
...
calls — they're prompt engineering that forces the model to reason.
...
### Layer 3: Guardrails (Reactive — When Layers 1 and 2 aren't enough)

Fire only when the model didn't follow prompt instructions or when the instructions
weren't sufficient. Detect failure patterns and inject correction prompts. Smart models
...
<a id="appendix-c"></a>
## Appendix C: Parallelization Assessment

### Can the migration phases run in parallel?

**Mostly no.** The constraint isn't uptime (no users) — it's:

1. **File contention:** `agent_factory.py` is touched by 4 of 5 phases. Parallel
   worktrees editing the same file produce merge conflicts that a third session must
   resolve without context for either change.

2. **Phase ordering:** Phase 3 (rewire loop) needs Phase 1 done first. Otherwise the
   session reads 1,400 lines of dead code to determine what's relevant. Phase 1 done
   first means a clean, smaller codebase for Phase 3.

3. **Integration testing:** Phased approach runs `make check` after each phase. Big-bang
   defers all integration issues to the end when no session has context for the pieces.

### What CAN parallelize

Two genuinely independent streams:

```
Stream A (structural):     Stream B (content):
  Phase 1: Delete            Write prompt templates
  Phase 2: New components      (reasoning.md, tool_guide.md, self_check.md)
  Phase 3: Rewire loop       Restructure obsidian-cli skill
  Phase 5: Procedural memory Update tool descriptions
  Phase 6: Cleanup
```

Stream B is pure content work with zero file contention against Stream A.

### Recommended: 3 Sessions (collapsed phases)

```
Session 1: Phase 1 + Phase 2 (delete + create standalone components)
Session 2: Phase 3 + Phase 4 (rewire loop + prompt architecture)
Session 3: Phase 5 + Phase 6 (procedural memory + cleanup)
```
