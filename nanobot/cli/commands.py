...
def gateway(
    ...
    agent = AgentLoop(
        ...
        observe_enabled=config.observe.enabled,
    )
...
def observe(
    ...
    """Start trace observe web server."""
    from nanobot.observe.web.server import serve_observe

    console.print(f"{__logo__} Observe UI: http://{host}:{port}")
    serve_observe(host=host, port=port)
