...
- 如果 AI 表格字段相关操作需要用户打开文档或重新授权，只返回链接、页面入口或简短说明。
- 如果执行过程中遇到 `AUTH_TOKEN_EXPIRED` / `USER_TOKEN_ILLEGAL` / "Token验证失败"，统一按 [global-reference.md](./global-reference.md) 中的认证规则处理；用户可见回复只返回授权链接和完成授权后的下一步提示。
