        ...
        *,
        ...
        try:
            ...
            if message_id is None or not text:
                return False
            if text == self._draft_last_text.get(chat_key) and reply_markup is None:
                self._draft_pending_text.pop(chat_key, None)
                return True
            if await self._edit_text_message(chat_id, message_id, text, reply_markup=reply_markup):
    ...
    async def send(self, msg: OutboundMessage) -> None:
        ...
        if not is_progress and chat_key in self._draft_messages:
            ...
            if can_finalize_draft:
                ...
                if task:
                    task.cancel()
                if await self._flush_draft_update(chat_key, chat_id, immediate=True, reply_markup=reply_markup):
