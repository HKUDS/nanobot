...
# Pattern to detect incomplete LaTeX (starts but doesn't end)
_LATEX_PENDING_RE = re.compile(r"\$\$?[^$]*$")  # incomplete $$ or $ formula
...
class _FeishuStreamBuf:
    ...
    sequence: int = 0
    last_edit: float = 0.0
    # Buffer for incomplete LaTeX formulas during streaming
    latex_pending: str = ""
...
class FeishuChannel(BaseChannel):
    ...
    def _process_streaming_latex(cls, text: str, pending: str) -> tuple[str, str, list[str]]:
        ...
        For streaming, formulas may arrive incrementally. This method:
        1. Appends pending text to complete partial formulas
        2. Detects complete formulas and renders them to images
        3. Returns (processed_text, new_pending, image_paths)
        ...
        has_trailing_dollar = combined.rstrip().endswith('$')
        has_trailing_double_dollar = combined.rstrip().endswith('$$')
        ...
        new_pending = ""
        if has_trailing_dollar and not images:
            # Check if we have an incomplete formula
            # Look for unclosed $ that isn't part of $$...$$
            processed_stripped = processed.rstrip()
            # Count unclosed single $ signs
