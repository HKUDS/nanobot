...
Most people do not need to edit JSON for the first run. The wizard handles the initial provider, model, and local WebUI settings. `nanobot webui` starts the gateway and opens the browser in one step. In a headless or terminal-only environment, run `nanobot gateway` instead. After the WebUI opens, use **Settings** for models and built-in capabilities, **Settings → Channels** for chat apps, and **Apps** for CLI App or MCP integrations.
