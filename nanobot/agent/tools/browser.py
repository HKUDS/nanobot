            ...
            if not self._browser:
                try:
                    ...
                    if any(
                        ...
                        for keyword in ["browser", "executable", "chromium", "not found"]
                    ):
    ...
    async def create_session(self, session_id: str | None = None) -> BrowserSession:
        ...
        async with self._lock:
            ...
            if len(self._sessions) >= self._max_contexts:
                raise RuntimeError(
                    f"Max browser contexts ({self._max_contexts}) reached. Please close some sessions."
                )
...
Screenshot returns base64-encoded image data."""
    ...
    def parameters(self) -> dict[str, Any]:
        return {
            ...
            "properties": {
                ...
                "checked": {"type": "boolean", "description": "Check state (for check action)"},
                "script": {
                    "type": "string",
    ...
    async def execute(self, **kwargs: Any) -> ToolResult | str:
        ...
        if action in ("launch", "close"):
            ...
            if session_id:
                ...
                # Use default session
                session = self._manager.get_session(self._default_session_id)
                if not session:
                    # Auto-create default session
                    try:
                        session = await self._manager.create_session(self._default_session_id)
                        self._setup_console_logging(session)
                        logger.info(
                            f"Auto-created default browser session: {self._default_session_id}"
                        )
                    except Exception as e:
                        return ToolResult(
                            content=f"Error creating default session: {str(e)}", images=None
                        )
    ...
    async def _handle_new_session(self, session_id: str | None) -> ToolResult:
        ...
        self._setup_console_logging(session)
        return ToolResult(content=f"Created new session: {session.id}", images=None)
    ...
    async def _handle_navigate(self, session: BrowserSession, **kwargs) -> ToolResult:
        ...
        return ToolResult(content=f"Navigated to {url}. Page title: {title}", images=None)
    ...
    async def _handle_console(self, session_id: str | None) -> ToolResult:
        ...
        formatted = "\n".join([f"[{log.get('type', 'log')}] {log.get('text', '')}" for log in logs])
