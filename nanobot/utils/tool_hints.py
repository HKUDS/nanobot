...
def _abbreviate_command(cmd: str, max_len: int = 40) -> str:
    """Abbreviate paths in a command string, then smart-truncate."""
    # Replace file paths with abbreviated versions
    abbreviated = _PATH_IN_CMD_RE.sub(
        lambda m: abbreviate_path(m.group(), max_len=25), cmd
    )
    if len(abbreviated) <= max_len:
        return abbreviated
    # Try to cut at chain separators to show the first command only
    suffix = " \u2026"
    for sep in (" && ", " | ", "; "):
        idx = abbreviated.find(sep)
        if 0 < idx <= max_len - len(suffix):
            return abbreviated[:idx] + suffix
