...
- Use `edit_file` only for small exact replacements in one file, with `old_text` copied from `read_file`; when editing a specific numbered line, pass that exact line as `line_hint`; otherwise use `occurrence` to select one match or `replace_all=true` to replace every match. These three selectors are mutually exclusive; add `expected_replacements` independently to verify the match count.
