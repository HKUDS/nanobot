...
def test_openrouter_sets_default_attribution_headers() -> None:
    spec = find_by_name("openrouter")
    with patch("janniebot.providers.openai_compat_provider.AsyncOpenAI") as mock_client:
    ...
    assert headers["HTTP-Referer"] == "https://github.com/derek14/janniebot"
    assert headers["X-OpenRouter-Title"] == "janniebot"
...
def test_openrouter_user_headers_override_default_attribution() -> None:
    ...
    with patch("janniebot.providers.openai_compat_provider.AsyncOpenAI") as mock_client:
        OpenAICompatProvider(
            ...
            default_model="anthropic/claude-sonnet-4-5",
            extra_headers={
                "HTTP-Referer": "https://janniebot.ai",
                "X-OpenRouter-Title": "Janniebot Pro",
    ...
    headers = mock_client.call_args.kwargs["default_headers"]
    assert headers["HTTP-Referer"] == "https://janniebot.ai"
    assert headers["X-OpenRouter-Title"] == "Janniebot Pro"
    assert headers["X-OpenRouter-Categories"] == "cli-agent,personal-agent"
...
async def test_openai_compat_stream_watchdog_returns_error_on_stall(monkeypatch) -> None:
    monkeypatch.setenv("JANNIEBOT_STREAM_IDLE_TIMEOUT_S", "0")
