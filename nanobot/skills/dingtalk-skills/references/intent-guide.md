...
- 本文中的命令示例和跨产品工作流仅供 agent 在后台执行参考，不要原样发给用户。
- 当路由到需要用户参与授权、打开页面或查看文档的场景时，只返回链接、页面入口或简短说明。
- 如果任一钉钉产品在执行中遇到 `AUTH_TOKEN_EXPIRED` / `USER_TOKEN_ILLEGAL` / "Token验证失败"，统一按 [global-reference.md](./global-reference.md) 中的认证规则处理；用户可见回复只返回授权链接和完成授权后的下一步提示。
