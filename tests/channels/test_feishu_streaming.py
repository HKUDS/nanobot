...
class TestSendDeltaResuming:
    """Tests for intermediate _stream_end with _resuming=True (multi-segment streaming)."""

    @pytest.mark.asyncio
    async def test_resuming_keeps_buffer(self):
        ch = _make_channel()
        ch._stream_bufs["oc_chat1"] = _FeishuStreamBuf(
            text="Segment 1", card_id="card_1", sequence=2, last_edit=0.0,
        )
        ch._client.cardkit.v1.card_element.content.return_value = _mock_content_response()
        ...
        assert "oc_chat1" in ch._stream_bufs
        buf = ch._stream_bufs["oc_chat1"]
        assert buf.text == "Segment 1"
        assert buf.card_id == "card_1"
        ch._client.cardkit.v1.card_element.content.assert_called_once()
        ch._client.cardkit.v1.card.settings.assert_not_called()

    @pytest.mark.asyncio
    async def test_resuming_without_card_id_is_noop(self):
        ch = _make_channel()
        ch._stream_bufs["oc_chat1"] = _FeishuStreamBuf(
            text="Segment 1", card_id=None, sequence=0, last_edit=0.0,
        )
    ...
    @pytest.mark.asyncio
    async def test_resuming_without_buf_is_noop(self):
        ch = _make_channel()
        await ch.send_delta("oc_chat1", "", metadata={"_stream_end": True, "_resuming": True})
        ch._client.cardkit.v1.card_element.content.assert_not_called()

    @pytest.mark.asyncio
    async def test_multi_segment_accumulates_then_finalizes(self):
        """Full multi-segment flow: deltas -> resuming end -> more deltas -> final end."""
        ch = _make_channel()
        ch._stream_bufs["oc_chat1"] = _FeishuStreamBuf(
            text="Part A", card_id="card_1", sequence=1, last_edit=0.0,
        )
        ch._client.cardkit.v1.card_element.content.return_value = _mock_content_response()
        ch._client.cardkit.v1.card.settings.return_value = _mock_content_response()
        ...
        buf.text += " Part B"
        buf.last_edit = 0.0
        ch._client.cardkit.v1.card_element.content.reset_mock()
