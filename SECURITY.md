...
8. **MessageTool Open Relay** - By default, `MessageTool` (`send_message`) only allows the agent to reply to the session that originated the request, and a guarded allowlist of additional destinations (`allow_destination()`). If allowlist entries are added programmatically without user consent, the agent could be used to relay messages to arbitrary channels. Mitigation: only call `allow_destination()` from audited code paths; review message routing configuration before enabling cross-channel messaging.
