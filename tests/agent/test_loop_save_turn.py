...
def test_restore_runtime_checkpoint_persists_pending_attachment_metadata() -> None:
    loop = _mk_loop()
    session = Session(
        key="test:checkpoint-attachment",
    ...
    assert restored is True
    assert session.messages[-2]["content"] == "first answer"
    assert session.messages[-1]["content"] == "read the follow-up"
    assert session.messages[-1]["media"] == ["/media/follow-up.csv"]
    assert "_meta" not in session.messages[-1]
    assert AgentLoop._RUNTIME_CHECKPOINT_KEY not in session.metadata


def test_runtime_checkpoint_keeps_injection_prefix_across_later_tool_checkpoint(
    tmp_path: Path,
) -> None:
    loop = _make_full_loop(tmp_path)
    session = loop.sessions.get_or_create("test:checkpoint-prefix")
        ...
        session,
        {
            ...
            "turn_suffix": [
                {"role": "assistant", "content": "first answer"},
                {
                    ...
                    "content": "follow-up",
                    "_meta": {
                        ATTACHMENT_MEDIA_MESSAGE_META: ["/media/follow-up.csv"],
                    },
                },
            ],
        },
    ...
    assert checkpoint["recovery_prefix"][1]["_meta"][
        ATTACHMENT_MEDIA_MESSAGE_META
    ] == ["/media/follow-up.csv"]
    ...
    assert session.messages[2]["media"] == ["/media/follow-up.csv"]
    assert session.messages[-1]["content"] == "attachment body"


def test_runtime_checkpoint_strips_pending_image_data_but_keeps_media(
    tmp_path: Path,
) -> None:
    loop = _make_full_loop(tmp_path)
    session = loop.sessions.get_or_create("test:checkpoint-image")
    image_path = str((tmp_path / "pending.png").resolve())
    loop._set_runtime_checkpoint(
        session,
        {
            "phase": "pending_injection",
            "turn_suffix": [
                {
                    ...
                    "_meta": {
                        ATTACHMENT_MEDIA_MESSAGE_META: [image_path],
                    },
                }
            ],
        },
    ...
    checkpoint = session.metadata[AgentLoop._RUNTIME_CHECKPOINT_KEY]
    assert "data:image" not in str(checkpoint)
    assert checkpoint["turn_suffix"][0]["content"] == [
        {"type": "text", "text": f"[image: {image_path}]"},
        {"type": "text", "text": "inspect this image"},
    ]
    assert checkpoint["turn_suffix"][0]["_meta"][
        ATTACHMENT_MEDIA_MESSAGE_META
    ] == [image_path]
                    ...
                    "content": [
                        {
                            "type": "image_url",
                            "image_url": {
                                "url": "data:image/png;base64,tool-payload",
                            },
                            "_meta": {"path": image_path},
                        }
                    ],
    ...
    checkpoint = session.metadata[AgentLoop._RUNTIME_CHECKPOINT_KEY]
    assert "data:image" not in str(checkpoint)
    assert checkpoint["completed_tool_results"][0]["content"] == [
        {"type": "text", "text": f"[image: {image_path}]"},
    ]
    ...
    assert restored is True
    assert session.messages[-3]["media"] == [image_path]
    assert session.messages[-1]["role"] == "tool"
    assert "data:image" not in str(session.messages)
...
@pytest.mark.asyncio
async def test_ephemeral_turn_leaves_persisted_session_unchanged(tmp_path: Path) -> None:
    loop = _make_full_loop(tmp_path)
    loop.consolidator.maybe_consolidate_by_tokens = AsyncMock(return_value=False)  # type: ignore[method-assign]
    attachment = tmp_path / "one-off.txt"
    attachment.write_text("one-off attachment", encoding="utf-8")
    session = loop.sessions.get_or_create("sdk:ephemeral")
    session.add_message("user", "persisted question")
    session.add_message("assistant", "persisted answer")
    session.metadata["keep"] = {"nested": ["value"]}
    loop.sessions.save(session)
    expected_messages = [dict(message) for message in session.messages]
    expected_metadata = {"keep": {"nested": ["value"]}}

    async def fake_run_agent_loop(initial_messages, **kwargs):
        assert kwargs["ephemeral"] is True
        assert kwargs["attachment_paths"] == [str(attachment)]
        return (
            "one-off answer",
            [],
            [*initial_messages, {"role": "assistant", "content": "one-off answer"}],
            "completed",
            False,
        )
    ...
    assert result.content == "one-off answer"
    loop.sessions.invalidate("sdk:ephemeral")
    persisted = loop.sessions.get_or_create("sdk:ephemeral")
    assert persisted.messages == expected_messages
    assert persisted.metadata == expected_metadata
