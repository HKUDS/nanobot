...
Option B / 15K is complete. Next choice: finish branch / commit / PR, or start the next CRM MCP data tool. Wait for further explicit user approval before deeper cleanup such as moving/deleting direct adapter code/tests or handling `nanobot/crm/real_smoke_diagnostics.py`. If the user wants a real CRM smoke result before deeper cleanup, first fix runtime config propagation outside chat so the required package-local smoke runtime config is visible, then rerun only the sanitized 15I smoke after explicit approval. Do not output raw GraphQL requests/responses, customer/project data, token, endpoint auth headers, Authorization/Bearer/cookie material, amount, address, or contact/free-text CRM content.
