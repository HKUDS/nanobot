...
class ExecTool(Tool):
    ...
    async def execute(self, command: str, working_dir: str | None = None, **kwargs: Any) -> str | ToolResult:
        ...
        try:
            ...
            # Truncate very long output for LLM context, but keep it larger than default
            max_llm_len = 10000
            if len(content) > max_llm_len:
                content = content[:max_llm_len] + f"\n... (truncated, {len(content) - max_llm_len} more chars)"
