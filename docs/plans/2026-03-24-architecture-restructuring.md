...
| **Cross-package imports** | Memory imports `nanobot.agent.prompt_loader` and `nanobot.agent.tracing` — these become cross-package; direction is correct (memory → agent utilities) |
