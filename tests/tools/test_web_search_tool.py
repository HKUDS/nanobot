...
from janniebot.agent.tools.web import WebSearchTool
from janniebot.config.schema import WebSearchConfig
...
async def test_brave_search(monkeypatch):
    async def mock_get(self, url, **kw):
        ...
        return _response(json={
            "web": {"results": [{"title": "JannieBot", "url": "https://example.com", "description": "AI assistant"}]}
        })

    monkeypatch.setattr(httpx.AsyncClient, "get", mock_get)
    tool = _tool(provider="brave", api_key="brave-key")
    result = await tool.execute(query="janniebot", count=1)
    assert "JannieBot" in result
...
async def test_duckduckgo_search(monkeypatch):
    ...
    monkeypatch.setattr("janniebot.agent.tools.web.DDGS", MockDDGS, raising=False)
    import janniebot.agent.tools.web as web_mod
