...
def test_delegation_config_wraps_sub_agent_config():
    """DelegationConfig composes SubAgentConfig and proxies its fields."""
    from nanobot.config.sub_agent import SubAgentConfig
    from nanobot.coordination.delegation import DelegationConfig

    sac = SubAgentConfig(workspace=Path("/tmp"), model="test")
    dc = DelegationConfig(
        sub_agent=sac,
        role_name="analyst",
        max_iterations=10,
    ...
    assert dc.sub_agent is sac
    assert dc.role_name == "analyst"
    assert dc.max_iterations == 10
    # Proxied attributes from sub_agent
    assert dc.workspace == Path("/tmp")
    assert dc.model == "test"
    assert dc.temperature == 0.7  # default
    assert dc.max_tokens == 4096  # default
