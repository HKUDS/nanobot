...
class TestResponsesConversationState:
    ...
    def test_server_compaction_preserves_bounded_retrieval_evidence(self):
        state = build_responses_state(
            provider="openai:test",
            model="gpt-5.6",
            input_items=[
                {"type": "message", "role": "user", "content": "find the evidence"},
                {
                    "type": "function_call",
                    "call_id": "call_search",
                    "name": "grep",
                    "arguments": '{"pattern":"revenue","path":"report.pdf"}',
                },
                {
                    "type": "function_call_output",
                    "call_id": "call_search",
                    "output": (
                        "report.pdf:42 [page=3,line=7]\n"
                        "> 42 [page=3,line=7]| confidential revenue details"
                    ),
                },
            ],
            output_items=[{"type": "compaction", "encrypted_content": "compact"}],
        )

        items = responses_state_items(state)
        assert items is not None
        assert len(items) == 2
        ledger = items[0]
        assert ledger["role"] == "developer"
        ledger_text = ledger["content"][0]["text"]
        assert ledger_text.startswith("nanobot.retrieval_evidence.v1\n")
        assert '"pattern":"revenue"' in ledger_text
        assert "report.pdf:42 [page=3,line=7]" in ledger_text
        assert '"result_sha256"' in ledger_text
        assert "confidential revenue details" not in ledger_text
        assert items[1] == {"type": "compaction", "encrypted_content": "compact"}

    def test_retrieval_ledger_merges_across_multiple_compactions(self):
        first = build_responses_state(
            provider="openai:test",
            model="gpt-5.6",
            input_items=[
                {
                ...
                },
                {
                    "type": "function_call_output",
                    "call_id": "call_search",
                    "output": "report.pdf:42 [page=3,line=7]\n> 42| revenue",
                },
            ],
            output_items=[{"type": "compaction", "encrypted_content": "first"}],
        )
        first_items = responses_state_items(first)
        assert first_items is not None

        second = build_responses_state(
            provider="openai:test",
            model="gpt-5.6",
            input_items=[
                *first_items,
                {
                    ...
                    "call_id": "call_read",
                    "name": "read_file",
                    "arguments": '{"path":"report.pdf","pages":"3"}',
                },
                {
                    "type": "function_call_output",
                    "call_id": "call_read",
            ...
            output_items=[{"type": "compaction", "encrypted_content": "second"}],
        )

        second_items = responses_state_items(second)
        assert second_items is not None
        ledger_text = second_items[0]["content"][0]["text"]
        assert '"tool":"grep"' in ledger_text
        assert '"tool":"read_file"' in ledger_text
        assert '"pages":"3"' in ledger_text
