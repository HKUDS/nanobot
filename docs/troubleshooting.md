...
| xAI login runs on a remote/headless machine | Run `nanobot provider login xai-grok` in an interactive terminal. If no browser opens, open the printed URL elsewhere and paste the final callback URL or authorization code when prompted. |
