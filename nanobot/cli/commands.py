...
def _make_provider(config: Config, agent_cfg: AgentDefaults | None = None):
    ...
    if backend == "openai_codex" or provider_name == "openai_codex" or model.startswith("openai-codex/"):
...
def _resolve_channel_agent_name(config: Config, channel_name: str) -> str:
    ...
    if isinstance(section, dict):
        ...
        selected = getattr(section, "agent", None)
...
def gateway(
    ...
    async def on_heartbeat_execute(tasks: str) -> str:
        ...
        session = target_agent.sessions.get_or_create("heartbeat")
        session.retain_recent_legal_suffix(hb_cfg.keep_recent_messages)
        target_agent.sessions.save(session)
