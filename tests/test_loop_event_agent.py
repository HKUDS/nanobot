"""Tests for AgentLoop._resolve_guest_agent_workspace — guest agent ACL routing."""
...
def _write_acl(workspace: Path, acl: dict) -> None:
    (workspace / "guest_agent_acl.json").write_text(
        json.dumps(acl), encoding="utf-8"
    )


def _make_guest_agent_workspace(workspace: Path, blocked_tools: list[str] | None = None) -> Path:
    guest_ws = workspace / "guest_agent"
    guest_ws.mkdir(exist_ok=True)
    (guest_ws / "SOUL.md").write_text("# Event Agent\n")
    if blocked_tools is not None:
        (guest_ws / "blocked_tools.json").write_text(json.dumps(blocked_tools))
    return guest_ws
...
def test_returns_none_when_no_acl_file(tmp_path: Path) -> None:
    ...
    assert loop._resolve_guest_agent_workspace("15551234567@s.whatsapp.net") is None
...
def test_matches_full_jid(tmp_path: Path) -> None:
    ...
    _make_guest_agent_workspace(tmp_path)
    loop = _mk_loop(tmp_path)
    result = loop._resolve_guest_agent_workspace("15551234567@s.whatsapp.net")
...
def test_matches_phone_prefix(tmp_path: Path) -> None:
    ...
    _make_guest_agent_workspace(tmp_path)
    loop = _mk_loop(tmp_path)
    result = loop._resolve_guest_agent_workspace("15551234567")
    assert result is not None


# ── guest_agent workspace missing despite ACL match ──────────────────────────

def test_returns_none_when_workspace_dir_missing(tmp_path: Path) -> None:
    _write_acl(tmp_path, {"15551234567@s.whatsapp.net": {"name": "Jake", "event_id": "trip"}})
    # guest_agent/ directory NOT created
    loop = _mk_loop(tmp_path)
    assert loop._resolve_guest_agent_workspace("15551234567@s.whatsapp.net") is None
...
def test_returns_none_on_corrupt_acl(tmp_path: Path) -> None:
    (tmp_path / "guest_agent_acl.json").write_text("not valid json", encoding="utf-8")
    loop = _mk_loop(tmp_path)
    assert loop._resolve_guest_agent_workspace("15551234567@s.whatsapp.net") is None
...
def test_result_is_cached(tmp_path: Path) -> None:
    ...
    _make_guest_agent_workspace(tmp_path)
    loop = _mk_loop(tmp_path)
    result1 = loop._resolve_guest_agent_workspace("15551234567@s.whatsapp.net")
    result2 = loop._resolve_guest_agent_workspace("15551234567@s.whatsapp.net")
    assert result1 is result2  # same tuple object from cache


def test_cache_is_per_workspace_path(tmp_path: Path) -> None:
    _write_acl(tmp_path, {"15551234567@s.whatsapp.net": {"name": "Jake", "event_id": "trip"}})
    guest_ws = _make_guest_agent_workspace(tmp_path)
    loop = _mk_loop(tmp_path)
    loop._resolve_guest_agent_workspace("15551234567@s.whatsapp.net")
    assert guest_ws in loop._guest_agent_cache
...
def test_second_guest_also_matched(tmp_path: Path) -> None:
    ...
    _make_guest_agent_workspace(tmp_path)
    loop = _mk_loop(tmp_path)
    assert loop._resolve_guest_agent_workspace("15551234567@s.whatsapp.net") is not None
    assert loop._resolve_guest_agent_workspace("15559876543@s.whatsapp.net") is not None
...
def test_non_guest_not_matched_with_multiple_guests(tmp_path: Path) -> None:
    ...
    _make_guest_agent_workspace(tmp_path)
    loop = _mk_loop(tmp_path)
    assert loop._resolve_guest_agent_workspace("99999999999@s.whatsapp.net") is None
...
def test_blocked_tools_loaded_from_workspace(tmp_path: Path) -> None:
    _write_acl(tmp_path, {"15551234567@s.whatsapp.net": {"name": "Jake", "event_id": "trip"}})
    _make_guest_agent_workspace(tmp_path, blocked_tools=["write_file", "edit_file", "gmail_fetch"])
    loop = _mk_loop(tmp_path)
    _, _, blocked = loop._resolve_guest_agent_workspace("15551234567@s.whatsapp.net")
    assert blocked == frozenset({"write_file", "edit_file", "gmail_fetch"})


def test_blocked_tools_empty_when_no_file(tmp_path: Path) -> None:
    _write_acl(tmp_path, {"15551234567@s.whatsapp.net": {"name": "Jake", "event_id": "trip"}})
    _make_guest_agent_workspace(tmp_path)  # no blocked_tools.json
    loop = _mk_loop(tmp_path)
    _, _, blocked = loop._resolve_guest_agent_workspace("15551234567@s.whatsapp.net")
...
def test_blocked_tools_empty_on_corrupt_file(tmp_path: Path) -> None:
    _write_acl(tmp_path, {"15551234567@s.whatsapp.net": {"name": "Jake", "event_id": "trip"}})
    guest_ws = _make_guest_agent_workspace(tmp_path)
    (guest_ws / "blocked_tools.json").write_text("not json")
    loop = _mk_loop(tmp_path)
    _, _, blocked = loop._resolve_guest_agent_workspace("15551234567@s.whatsapp.net")
    assert blocked == frozenset()
