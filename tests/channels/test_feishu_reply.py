...
async def test_on_message_audio_publishes_downloaded_path_and_transcription() -> None:
    ...
    channel._download_and_save_media = AsyncMock(
        return_value=(r"C:\\Users\\dodre\\.blackcat\\media\\feishu\\voice.ogg", "[audio: voice.ogg]")
    )
    ...
    channel.transcribe_audio.assert_awaited_once_with(r"C:\\Users\\dodre\\.blackcat\\media\\feishu\\voice.ogg")
    assert len(captured) == 1
    assert captured[0].media == [r"C:\\Users\\dodre\\.blackcat\\media\\feishu\\voice.ogg"]
