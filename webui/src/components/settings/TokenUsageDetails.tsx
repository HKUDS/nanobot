...
export function TokenUsageDetails({ days, models, modelDays, loadDetails }: {
  ...
  return (
    <DialogLayoutContext.Provider value={null}>
      <Dialog open={open} onOpenChange={setOpen}>
        ...
        <DialogContent className="max-h-[calc(100dvh-32px)] w-[min(1100px,calc(100vw-32px))] max-w-none gap-5 overflow-hidden p-5 pt-10 sm:p-7 sm:pt-10" aria-describedby={undefined}>
          <DialogTitle className="sr-only">{t("settings.usage.shortTitle")}</DialogTitle>
          <div className="min-h-0 max-h-[calc(100dvh-120px)] overflow-y-auto pr-1" data-usage-details-scroll>
          {loadDetails ? (open ? <UsageDashboard load={loadDetails} /> : null) : <>
