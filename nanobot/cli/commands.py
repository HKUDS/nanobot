...
@app.command()
def gateway_token():
    """Display the current gateway authentication token."""
    ...
    try:
        ...
        if token:
            console.print(f"[green]Gateway token:[/green] {token}")
        else:
            console.print("[yellow]No token configured. Run 'nanobot onboard' first.[/yellow]")
...
def gateway(
    ...
    # Start HTTP server for web UI if token is configured
