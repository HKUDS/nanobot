...
async def wait_for_bff() -> bool:
    print(f"[等待BFF] 等待BFF服务启动... (timeout={WAIT_TIMEOUT}s)")
    start = time.time()
...
async def wait_for_km_ready() -> bool:
    print("[等待KM] 等待KM容器启动... (timeout=30.0s)")
    start = time.time()
    ...
    try:
        async with httpx.AsyncClient(timeout=30.0) as client:
            resp = await client.post(
                ...
                json={"content": skill_content, "skill_version": "1.0"}
            )
            if resp.status_code == 200:
                print("[预置Skill] ✅ 成功")
                return True
            else:
                print(f"[预置Skill] ❌ 失败: {resp.status_code}")
                return False
    except Exception as e:
        print(f"[预置Skill] ❌ 异常: {e}")
        return False


async def create_collab_container(name: str) -> Optional[Dict]:
    print(f"[创建协作者] 创建 {name}...")
    try:
        async with httpx.AsyncClient(timeout=60.0) as client:
            resp = await client.post(
                f"{BFF_BASE_URL}/conversations",
                json={"title": name, "model": "deepseek-chat", "agent_type": "collab"}
            )
            if resp.status_code == 200:
                data = resp.json()
                conv_id = data.get("conversation_id", "")
                port = data.get("container_port", 0)
                print(f"[创建协作者] {name}: {conv_id[:8]}, port={port}")
                return {"conversation_id": conv_id, "container_port": port, "name": name}
            else:
                print(f"[创建协作者] {name} 失败: {resp.status_code}")
                return None
    except Exception as e:
        print(f"[创建协作者] {name} 异常: {e}")
        return None
        ...
        for attempt in range(max_retries):
            try:
                async with httpx.AsyncClient(timeout=10.0) as client:
                    ...
                    if not km_url:
                        continue
                    parse_prompt = f"请从以下内容中提取JSON对象，返回包含page_content的JSON：\n{raw_content[:2000]}"
                    ...
                    parse_content = parse_data.get("content", "")
                    print(f"  [KM解析] 返回内容长度: {len(parse_content)}")
                    parsed = parse_json_response(parse_content, "KM解析")
...
async def call_llm_merge(results: List[Dict], round_idx: int) -> str:
    """调用KM容器合并多个Agent的学习结果（优化Prompt方案）"""
    merge_prompt = f"""【重要指令：请直接输出合并后的知识内容，不要输出任何JSON、状态消息、解释或额外文本。只输出合并后的纯文本内容。】

请合并以下{len(results)}个Agent在第{round_idx}轮的学习结果，提炼出最完整、准确的知识内容：
...
"""
    for i, r in enumerate(results):
        ...
        role = r.get("role", "未知")
        if pc:
            merge_prompt += f"## Agent{i+1} ({role})\n{pc}\n\n"
    merge_prompt += "请输出合并后的知识内容（纯文本，不要JSON格式）："
    try:
        # 先获取KM容器URL
        async with httpx.AsyncClient(timeout=10.0) as client:
            km_resp = await client.get(f"{BFF_BASE_URL}/knowledge-manager/km-url")
        ...
        async with httpx.AsyncClient(timeout=120.0) as client:
            ...
            raw_response = merge_resp.json().get("content", "")
            
            # 增强解析：尝试提取纯文本内容
            merged = extract_merged_content(raw_response)
...
def extract_merged_content(raw_response: str) -> str:
    """从KM返回的内容中提取合并后的知识"""
    if not raw_response:
        return ""
    
    # 策略1：尝试解析JSON并提取content字段
    try:
        data = json.loads(raw_response)
        if isinstance(data, dict):
            # 尝试多个可能的字段名
            for key in ["content", "merged_content", "text", "result"]:
...
async def agent_round_with_lock(agent_info: Dict, round_prompt: Dict, file_lock: FileLock, round_idx: int) -> Dict:
    ...
    chat_resp = await chat_with_agent(conv_id, full_prompt, agent_info['container_port'])
    ...
    if chat_resp.get("success"):
        ...
        if not content:
            print(f"  [{agent_name}] [调试] content为空，raw_resp={chat_resp}")
...
async def run_group_b_baseline():
    """运行B组Baseline测试（5轮，与A组对比）"""
    print("\n" + "=" * 70)
    print("B组Baseline：5轮学习任务（与A组对比）")
    print("=" * 70)
...
""")
    ...
    print("\n[Step 2] 创建10个协作者Agent")
    agents = []
    for i in range(10):
        info = await create_collab_container(f"b_baseline_{i+1}")
        if info:
    ...
    report = {
        "group": "B_baseline",
        "description": "B组Baseline：5轮学习任务（与A组对比）",
    ...
    report_path = os.path.join(DATA_DIR, "b_group_baseline_report.json")
...
if __name__ == "__main__":
    asyncio.run(run_group_b_baseline())
