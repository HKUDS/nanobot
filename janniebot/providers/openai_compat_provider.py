...
from janniebot.providers.base import LLMProvider, LLMResponse, ToolCallRequest

if TYPE_CHECKING:
    from janniebot.providers.registry import ProviderSpec
...
_DEFAULT_OPENROUTER_HEADERS = {
    "HTTP-Referer": "https://github.com/derek14/janniebot",
    "X-OpenRouter-Title": "janniebot",
...
def _uses_openrouter_attribution(spec: "ProviderSpec | None", api_base: str | None) -> bool:
    """Apply janniebot attribution headers to OpenRouter requests by default."""
...
class OpenAICompatProvider(LLMProvider):
    ...
    def _build_kwargs(
        ...
        if spec:
            prefix = model_name.split("/", 1)[0].lower().replace("-", "_")
            if prefix == spec.name:
                model_name = model_name.split("/", 1)[1]
            elif spec.strip_model_prefix:
    ...
    def _parse(self, response: Any) -> LLMResponse:
        ...
        sdk_tool_calls: list[Any] = []
        for ch in response.choices:
            m = ch.message
            if hasattr(m, "tool_calls") and m.tool_calls:
                sdk_tool_calls.extend(m.tool_calls)
    ...
    def _parse_chunks(cls, chunks: list[Any]) -> LLMResponse:
        ...
        def _coerce_stream_tool_args(arg_str: str) -> dict[str, Any]:
            raw = json_repair.loads(arg_str) if arg_str else {}
        ...
        return LLMResponse(
            ...
            tool_calls=[
                ToolCallRequest(
                    ...
                    name=b["name"],
                    arguments=_coerce_stream_tool_args(b["arguments"]),
    ...
    async def chat_stream(
        ...
        idle_timeout_s = int(os.environ.get("JANNIEBOT_STREAM_IDLE_TIMEOUT_S", "90"))
        try:
