"""Utility functions for janniebot."""
...
_TOOL_RESULTS_DIR = ".janniebot/tool-results"
...
def split_message(content: str, max_len: int = 2000) -> list[str]:
    ...
    Args:
        ...
        max_len: Maximum length per chunk (default 2000; typical platform limit).
...
def build_status_content(
    *,
    ...
    return "\n".join([
        f"\U0001f408 janniebot v{version}",
...
def sync_workspace_templates(workspace: Path, silent: bool = False) -> list[str]:
    ...
    try:
        tpl = pkg_files("janniebot") / "templates"
    except Exception:
        return []
    if not tpl.is_dir():
        return []
