...
from nanobot.api.kosmos import database, EventBroadcaster
...
class KosmosServer(EventBroadcaster):
    ...
    async def broadcast_event(self, event_type: str, payload: Any):
        ...
        logger.debug("Broadcast {} to {} clients", event_type, len(self.clients) - len(dead_clients))
    ...
    async def websocket_handler(self, ws: WebSocketServerProtocol, path: str):
        ...
        try:
            ...
            if self.db:
                projects = await self.db_ops.get_projects_with_task_count(self.db, include_hidden=True)
                await ws.send(json.dumps({
                    "type": "project_status",
                    "payload": projects,
                }))
    ...
    async def start(self):
        ...
        ws_runner = websockets_runner = None
