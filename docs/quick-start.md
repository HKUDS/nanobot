...
universal fallback). While a turn is running, `Enter` queues a follow-up instead. `Option+Up` on
macOS (`Alt+Up` on Windows/Linux) edits the latest queued message. Exit
