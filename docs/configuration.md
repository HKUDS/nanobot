...
| `gateway.heartbeat.isolatedSession` | `true` | Use a dedicated heartbeat session. Set to `false` to run in the target chat session with its conversation context; that shared session is not pruned by heartbeat. |
