...
and several io.net model families (`deepseek-ai/*`, `qwen/*`, `google/*`, `mistralai/*`)
match other providers' routing keywords — set `"provider": "ionet"` in the preset
whenever other providers are configured (with only one provider configured, plain
model IDs route to it).
