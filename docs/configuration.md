...
Failover normally runs when a candidate returns a model/provider error before any answer text has been streamed. Stream-stall timeouts are the recovery exception: if the provider already emitted partial answer text and then stalls, nanobot closes the current stream segment and retries/fails over in a new segment. Typical fallback cases include timeouts, connection errors, 5xx server errors, 429 rate limits, overloads, and model availability errors. Authentication, permission, quota, and balance errors skip later candidates that resolve to the same provider account and endpoint, then continue at the first independent provider failure domain. Failover stops for malformed requests, content filtering/refusals, and context-length/message-format errors because another provider should not reinterpret an invalid or rejected request.
