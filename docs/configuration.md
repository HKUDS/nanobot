...
| `tools.exec.pathPrepend` | `""` | Extra directories to prepend to `PATH` for the executed command. Use this when configured tools should win executable lookup precedence, such as a Python virtual environment's `bin` or `Scripts` directory. This does not affect sandbox launcher selection. |
