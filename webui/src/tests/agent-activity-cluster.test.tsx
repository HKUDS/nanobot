...
describe("AgentActivityCluster", () => {
  ...
  it("keeps estimated pending diff stats out of the thinking header", async () => {
    const restoreMotion = installReducedMotion();
    try {
      render(
        <AgentActivityCluster
      ...
      const header = screen.getByRole("button", { name: /preparing edit/i });
      expect(header).toHaveTextContent("Thinking");
      expect(header).not.toHaveTextContent("+16");
  ...
  it("renders the resolved file row instead of an orphan pending placeholder", () => {
    render(
      <AgentActivityCluster
        messages={activityMessages("", {
          id: "t2",
    ...
    expect(screen.queryByText("Preparing file edit…")).not.toBeInTheDocument();
    expect(screen.getByTestId("activity-file-reference")).toHaveTextContent("project-plan.md");
    expect(screen.getAllByText("+206").length).toBeGreaterThan(0);
  });
