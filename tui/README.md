...
resume command with any explicit configuration and workspace selectors, so it resumes the current
session in the same context. Paths use POSIX-shell quoting on macOS/Linux and PowerShell quoting on
Windows. When earlier transcript pages exist, press
`PageUp` at the top to load them in place.
