...
from janniebot.config.schema import ChannelsConfig
from janniebot.utils.restart import RestartNotice
...
def test_discover_plugins_loads_entry_points():
    from janniebot.channels.registry import discover_plugins
...
def test_channels_login_uses_discovered_plugin_class(monkeypatch):
    ...
    monkeypatch.setattr("janniebot.config.loader.load_config", lambda config_path=None: Config())
    monkeypatch.setattr(
        "janniebot.channels.registry.discover_all",
...
async def test_send_with_retry_retries_on_failure():
    ...
    with patch("janniebot.channels.manager.asyncio.sleep", new_callable=AsyncMock) as mock_sleep:
        await mgr._send_with_retry(mgr.channels["failing"], msg)
...
async def test_send_with_retry_no_retry_when_max_is_zero():
    ...
    with patch("janniebot.channels.manager.asyncio.sleep", new_callable=AsyncMock):
        await mgr._send_with_retry(mgr.channels["failing"], msg)
...
async def test_send_with_retry_propagates_cancelled_error_during_sleep():
    ...
    with patch("janniebot.channels.manager.asyncio.sleep", side_effect=cancel_during_sleep):
        with pytest.raises(asyncio.CancelledError):
            await mgr._send_with_retry(mgr.channels["failing"], msg)
...
async def test_enabled_channels_returns_channel_names():
    ...
    mgr.channels = {
        ...
        "whatsapp": _StartableChannel(fake_config, mgr.bus),
    }
    mgr._dispatch_task = None
...
async def test_notify_restart_done_enqueues_outbound_message():
    ...
    with patch("janniebot.channels.manager.consume_restart_notice_from_env", return_value=notice):
