...
def login_gemini_cli_oauth(use_manual_flow: bool = False) -> GeminiOAuthCredentials:
    """Perform Gemini CLI OAuth login flow.

    Args:
        use_manual_flow: If True, use manual URL paste flow instead of local callback
    ...
    # Check if running in remote environment (WSL2, VPS, etc.)
    is_wsl = os.path.exists("/proc/version") and "microsoft" in Path(
        "/proc/version"
    ).read_text().lower()

    needs_manual = use_manual_flow or is_wsl

    if needs_manual:
        _get_console().print(
            "\n[cyan]Remote/VPS Environment Detected[/cyan]\n"
            "A URL will be shown for you to open in your LOCAL browser.\n"
            "After signing in, copy the redirect URL and paste it back here.\n"
        )
    else:
        _get_console().print(
            "\n[cyan]Browser will open for Google authentication.[/cyan]\n"
    ...
    pkce = generate_pkce()
    auth_url = build_auth_url(pkce["challenge"], pkce["verifier"])

    _get_console().print(f"\n[green]OAuth URL ready[/green]\n")

    if needs_manual:
        # Manual flow
        _get_console().print(f"Open this URL in your LOCAL browser:\n\n[bold]{auth_url}[/bold]\n")

        callback_input = _get_prompt()("Paste the redirect URL here: ")
        parsed = parse_callback_input(callback_input, pkce["verifier"])

        if "error" in parsed:
            raise RuntimeError(parsed["error"])

        if parsed["state"] != pkce["verifier"]:
            raise RuntimeError("OAuth state mismatch - please try again")

        _get_console().print("[dim]Exchanging authorization code for tokens...[/dim]")
        return exchange_code_for_tokens(parsed["code"], pkce["verifier"])

    else:
        # Automatic flow with local callback
        try:
            if webbrowser:
                webbrowser.open(auth_url)
        except Exception:
            _get_console().print(f"Open this URL in your browser:\n\n[bold]{auth_url}[/bold]\n")

        try:
            result = wait_for_local_callback(pkce["verifier"])
            _get_console().print("[dim]Exchanging authorization code for tokens...[/dim]")
            return exchange_code_for_tokens(result["code"], pkce["verifier"])

        except Exception as e:
            error_msg = str(e)
            if any(
                keyword in error_msg
                for keyword in ["EADDRINUSE", "port", "listen", "address already in use"]
            ):
                _get_console().print(
                    "[yellow]Local callback server failed. Switching to manual mode...[/yellow]"
                )
                _get_console().print(
                ...
                if "error" in parsed:
                    raise RuntimeError(parsed["error"]) from e

                if parsed["state"] != pkce["verifier"]:
                    raise RuntimeError("OAuth state mismatch - please try again") from e

                _get_console().print(
                    "[dim]Exchanging authorization code for tokens...[/dim]"
                )
                return exchange_code_for_tokens(parsed["code"], pkce["verifier"])
...
def save_token_to_disk(credentials: GeminiOAuthCredentials) -> Path:
    """Save OAuth token to disk for use with oauth-cli-kit.

    Args:
        credentials: GeminiOAuthCredentials instance

    Returns:
        Path to saved token file
    """
    token_dir = Path.home() / ".nanobot" / "tokens"
    token_dir.mkdir(parents=True, exist_ok=True)

    token_file = token_dir / "google_gemini_cli.json"
