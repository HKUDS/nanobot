...
async def get_settings(db: aiosqlite.Connection) -> dict[str, Any]:
    ...
    async with db.execute("SELECT key, value FROM settings") as cursor:
        rows = await cursor.fetchall()
        return {row["key"]: json.loads(row["value"]) if row["value"].startswith("{") else row["value"] for row in rows}
...
async def get_tasks_with_comment_count(db: aiosqlite.Connection, project_id: Optional[str] = None) -> list[dict[str, Any]]:
