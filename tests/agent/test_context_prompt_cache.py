...
async def test_recent_history_truncated_at_max_tokens(tmp_path) -> None:
    """Recent History section must be truncated to _MAX_HISTORY_CHARS."""
    import tiktoken

    workspace = _make_workspace(tmp_path)
    builder = ContextBuilder(workspace)

    big_entry = "word " * (builder._MAX_HISTORY_CHARS + 5_000)
    ...
    assert len(enc.encode(history_section[1])) <= builder._MAX_HISTORY_CHARS
