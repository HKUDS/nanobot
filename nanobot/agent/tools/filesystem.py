...
def _resolve_path(
    path: str, workspace: Path | None = None, allowed_dirs: list[Path] | Path | None = None
) -> Path:
    ...
    # Normalize allowed_dirs to a list
    allowed_list: list[Path] | None = None
    if allowed_dirs is not None:
        if isinstance(allowed_dirs, Path):
            allowed_list = [allowed_dirs.resolve()]
        else:
            allowed_list = [d.resolve() for d in allowed_dirs]
...
class _FsTool(Tool):
    ...
    def __init__(self, workspace: Path | None = None, allowed_dirs: list[Path] | Path | None = None):
