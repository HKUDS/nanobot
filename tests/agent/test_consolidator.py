...
class TestEagerConsolidation:
    async def test_disabled_eager_consolidation_is_noop(self, consolidator, runtime):
        ...
        await consolidator.maybe_eager_consolidate(session, runtime=runtime)

        consolidator.archive.assert_not_awaited()
        assert session.last_eager_consolidated == 0

    async def test_eager_consolidation_archives_without_trimming_session(
    ...
    async def test_eager_consolidation_skips_internal_sessions(
        self, consolidator, runtime
    ):
        consolidator.eager_consolidation = True
...
class TestConsolidatorTokenBudget:
    ...
    async def test_token_consolidation_skips_eager_archived_prefix(
        self, consolidator, runtime
    ):
        consolidator._SAFETY_BUFFER = 0
