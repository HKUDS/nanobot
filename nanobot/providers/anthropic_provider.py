...
_LONG_LIVED_TOKEN_SECONDS = 31536000
...
def _token_storage():
    ...
    return FileTokenStorage(
        token_filename="anthropic-oauth.json",
        app_name="nanobot",
        import_codex_cli=False,
    )


def get_anthropic_oauth_storage():
    """Return nanobot's Anthropic OAuth token storage."""
    return _token_storage()


def get_anthropic_oauth_login_status() -> Any | None:
    """Return an available Claude Code OAuth token-like object, if configured."""
    ...
    if env_token:
        ...
        return OAuthToken(
            access=env_token,
            refresh="",
            expires=int((time.time() + _LONG_LIVED_TOKEN_SECONDS) * 1000),
            account_id="CLAUDE_CODE_OAUTH_TOKEN",
        )

    token = None
    with suppress(Exception):
        token = _token_storage().load()
    if token and getattr(token, "access", None):
        return token
    return None


def login_anthropic_oauth(prompt_fn: Callable[[str], str] | None = None) -> Any:
    """Persist a Claude Code OAuth token generated by ``claude setup-token``."""
    existing = get_anthropic_oauth_login_status()
    if existing and getattr(existing, "access", None):
        return existing

    prompt = prompt_fn or input
    token_value = prompt("Paste CLAUDE_CODE_OAUTH_TOKEN from `claude setup-token`").strip()
    if not token_value:
        raise RuntimeError("No Anthropic OAuth token provided.")
...
class AnthropicProvider(LLMProvider):
    ...
    def _strip_prefix(model: str) -> str:
        if model.startswith(("anthropic-oauth/", "anthropic_oauth/")):
...
class AnthropicOAuthProvider(AnthropicProvider):
    """Anthropic Messages provider authenticated by Claude Code OAuth."""
    ...
    def __init__(self, default_model: str = "anthropic-oauth/claude-sonnet-4-20250514"):
        ...
        if not (token and getattr(token, "access", None)):
            raise RuntimeError(
                "Anthropic OAuth is not configured. Run `claude setup-token`, then either set "
                "CLAUDE_CODE_OAUTH_TOKEN or run `nanobot provider login anthropic-oauth`."
            )
        super().__init__(
            api_key=None,
            default_model=default_model,
            extra_headers={"anthropic-beta": _OAUTH_BETA_HEADER},
