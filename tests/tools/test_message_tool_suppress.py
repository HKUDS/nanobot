...
class TestMessageToolSuppressLogic:
    ...
    @pytest.mark.asyncio
    async def test_injected_followup_with_message_tool_suppresses_leaked_markup_notice(
        self, tmp_path: Path
    ) -> None:
        """Companion to the empty-fallback case above: once MessageTool has
        already sent real content this turn, a leaked-tool-call-markup
        fallback notice is suppressed the same way an empty response is,
        even when a mid-turn injection occurred."""
        loop = _make_loop(tmp_path)
        tool_call = ToolCallRequest(
            id="call1", name="message",
            arguments={"content": "Tool reply", "channel": "feishu", "chat_id": "chat123"},
