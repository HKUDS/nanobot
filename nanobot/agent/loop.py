...
class AgentLoop:
    ...
    async def _process_message(self, msg: InboundMessage) -> OutboundMessage | None:
        ...
        # 检查是否需要触发自动总结
        interval = int(os.environ.get("NANOBOT_SUMMARY_INTERVAL", "10"))
        ...
        if enabled and session.message_count > 0 and session.message_count % interval == 0:
            ...
            self.sessions.save(session)
            # 异步触发总结，不阻塞主流程
            asyncio.create_task(self._trigger_summary(session.key))
    ...
    async def _trigger_summary(self, session_key: str) -> None:
        ...
        try:
            ...
            logger.error(f"Auto-summary failed for {session_key}: {e}")
