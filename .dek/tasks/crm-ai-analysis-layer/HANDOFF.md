...
- Task 16B added `crm_mcp_server/crm_mcp_server/daily_report.py` with validation-before-reader-calls, injected dependency readers, deterministic metrics for project count, business chance count, status/apply-status distributions, due-today count, overdue count, sanitized unavailable metrics, deduped source refs, and allow-listed diagnostics.
...
- 16B verification passed: focused `uv run --extra dev pytest crm_mcp_server/tests/test_daily_report_facts.py` reported `17 passed in 0.02s`; full `uv run --extra dev pytest crm_mcp_server/tests` reported `83 passed in 0.06s`; `uv run --extra dev ruff check crm_mcp_server` reported `All checks passed!`; requested plain `python` safety assertion could not start because `python` is not on PATH, and the same assertion via `uv run python` reported `16B source safety assertions passed`.
...
16B is complete. Next recommendation: implement `crm_generate_weekly_report_facts` using the same mocked dependency-reader pattern, or push/open PR for the 16B branch. Wait for further explicit user approval before deeper cleanup such as moving/deleting direct adapter code/tests or handling `nanobot/crm/real_smoke_diagnostics.py`. If the user wants a real CRM smoke result before deeper cleanup, first fix runtime config propagation outside chat so the required package-local smoke runtime config is visible, then rerun only the sanitized 15I smoke after explicit approval. Do not output raw GraphQL requests/responses, customer/project data, token, endpoint auth headers, Authorization/Bearer/cookie material, amount, address, or contact/free-text CRM content.
