...
import blackcat.utils.tokens as tokens_module
...
async def test_prompt_above_threshold_triggers_consolidation(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(tokens_module, "estimate_message_tokens", lambda _message: 500)
...
async def test_prompt_above_threshold_archives_until_next_user_boundary(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(tokens_module, "estimate_message_tokens", lambda message: token_map[message["content"]])
...
async def test_consolidation_loops_until_target_met(tmp_path, monkeypatch) -> None:
    ...
    loop.consolidator._estimate_tokens = lambda _m: 100  # type: ignore[assignment]
...
async def test_consolidation_persists_summary_for_next_prepare_session(tmp_path, monkeypatch) -> None:
    ...
    loop.consolidator.estimate_session_prompt_tokens = mock_estimate  # type: ignore[method-assign]
    loop.consolidator._estimate_tokens = lambda _m: 150  # type: ignore[assignment]
    monkeypatch.setattr(tokens_module, "estimate_message_tokens", lambda _m: 150)
...
async def test_preflight_consolidation_before_llm_call(tmp_path, monkeypatch) -> None:
    ...
    loop.sessions.save(session)
    monkeypatch.setattr(tokens_module, "estimate_message_tokens", lambda _m: 500)
    ...
    loop.consolidator.estimate_session_prompt_tokens = mock_estimate  # type: ignore[method-assign]
    loop.consolidator._estimate_tokens = lambda _m: 500  # type: ignore[assignment]
    monkeypatch.setattr(tokens_module, "estimate_message_tokens", lambda _m: 500)
