...
""")
    ...
    agents = []
    for i in range(5):
        info = await create_collab_container(f"b_group_collab_{i+1}")
        if info:
    ...
    round_prompt = ROUNDS_PROMPTS[0]
    print(f"\n[B组] 第1轮开始: {round_prompt['title']}")
    round_start = time.perf_counter()
    
    tasks = []
    for agent in agents:
        task = asyncio.create_task(agent_round_with_lock(agent, round_prompt, file_lock, 1))
        tasks.append(task)
    ...
    all_inference_times = [r.get("inference_time", 0) for r in round_results]
    all_write_times = [r.get("write_time", 0) for r in round_results]
    
    print(f"\n[B组] 第1轮: 共{len(round_results)}个结果")
    ...
    if valid_results:
        merged_content = await call_llm_merge(valid_results, 1)
        ...
        print(f"  [B组] 第1轮合并完成: {merge_time:.2f}s")
        print(f"\n[合并结果预览]\n{merged_content[:500]}...")
    else:
        print(f"  [B组] 第1轮无有效结果，跳过合并")
    
    print(f"\n[B组] 第1轮总耗时: {round_time:.2f}s")
    ...
    report_path = os.path.join(DATA_DIR, "b_group_lite_report.json")
