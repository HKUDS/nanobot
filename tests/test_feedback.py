...
class TestFeedbackTool:
    ...
    @pytest.fixture()
    def db(self, tmp_path: Path) -> MemoryDatabase:
        return MemoryDatabase(tmp_path / "memory.db", dims=8)

    @pytest.fixture()
    def tool(self, db: MemoryDatabase) -> FeedbackTool:
    ...
    async def test_positive_feedback(self, tool: FeedbackTool, db: MemoryDatabase) -> None:
        result = await tool.execute(rating="positive", comment="great answer")
        assert result.success
        assert "positive" in result.output.lower()

        # Verify persisted in database
        events = db.event_store.read_events(type="feedback")
        assert len(events) == 1
        event = events[0]
    ...
    async def test_minimal_feedback(self, tool: FeedbackTool, db: MemoryDatabase) -> None:
    ...
    async def test_multiple_events_appended(self, tool: FeedbackTool, db: MemoryDatabase) -> None:
        ...
        events = db.event_store.read_events(type="feedback", limit=100)
        assert len(events) == 3
...
def _insert_feedback(db: MemoryDatabase, events: list[dict[str, Any]]) -> None:
...
class TestFeedbackSummary:
    ...
    def test_empty_db(self, db: MemoryDatabase) -> None:
        assert feedback_summary(db) == ""

    def test_load_filters_by_type(self, db: MemoryDatabase) -> None:
        _insert_feedback(
            db,
            [
    ...
    def test_summary_counts(self, db: MemoryDatabase) -> None:
        _insert_feedback(
            db,
            [
                {"type": "feedback", "rating": "positive"},
    ...
    def test_summary_includes_corrections(self, db: MemoryDatabase) -> None:
        _insert_feedback(
            db,
            [
                {
    ...
    def test_summary_topic_frequency(self, db: MemoryDatabase) -> None:
        _insert_feedback(
            db,
            [
    ...
    def test_non_feedback_events_ignored_in_summary(self, db: MemoryDatabase) -> None:
        _insert_feedback(
            db,
            [
