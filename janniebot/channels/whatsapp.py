...
class WhatsAppChannel(BaseChannel):
    ...
    async def login(self, force: bool = False) -> bool:
        ...
        """
        from janniebot.config.paths import get_runtime_subdir
        ...
        try:
            subprocess.run(
                [npm_cmd, "start"], cwd=bridge_dir, check=True, env=env
            )
            ...
            try:
                async with websockets.connect(bridge_url) as ws:
                    ...
                    async for message in ws:
                        try:
                            msg_text = message.decode("utf-8") if isinstance(message, bytes) else str(message)
                            await self._handle_bridge_message(msg_text)
...
def _ensure_bridge_setup() -> Path:
    ...
    if not source:
        raise RuntimeError(
            ...
            "Try reinstalling: pip install --force-reinstall janniebot"
        )
