...
```bash
python -m pip install "blackcat-ai[api]"
blackcat agent -m "Hello!"
blackcat serve
```

Run the CLI check first. If `blackcat agent -m "Hello!"` fails, fix provider or config setup before debugging the API server. By default, the API binds to `127.0.0.1:8900`. You can change this in `config.json`.
