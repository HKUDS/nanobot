---
name: update-setup
description: One-time setup wizard for the blackcat upgrade skill. Triggers: setup update, configure update, 切设置更新, 初始化更新.
---
...
Use `exec` to run `blackcat --version`. Tell the user the current version.

Then collect install clues with `exec`. These commands are best-effort; if one fails,
keep going and show the useful output:

```
command -v blackcat || true
python -m pip show blackcat-ai || true
pipx list | sed -n '/blackcat-ai/,+3p' || true
uv tool list | sed -n '/blackcat-ai/,+3p' || true
```
...
answer, stop and ask the user to rerun this setup when they know how blackcat was
installed.
...
`question: "Where is your blackcat source checkout? Enter an absolute path or a path relative to this workspace:"`.
...
| Method | Command |
|--------|---------|
| uv | `uv tool install "blackcat-ai[EXTRAS]" --force` |
| pipx | `pipx install --force "blackcat-ai[EXTRAS]"` |
| pip | `python -m pip install --upgrade "blackcat-ai[EXTRAS]"` |
...
---
name: update
description: "Upgrade blackcat to the latest version. Triggers: upgrade blackcat, update blackcat, 升级blackcat, 更新blackcat."
---
...
4. Use `exec` to verify: `blackcat --version`
5. Tell the user the new version. Say: "Run `/restart` to restart blackcat and apply the update. If `/restart` is unavailable in this channel, restart the blackcat process manually."
```

## Step 5: Confirm

Tell the user: "Upgrade skill created. Say 'upgrade blackcat' when you want to update."
