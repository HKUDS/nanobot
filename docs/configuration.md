...
>
> This differs from the **token-driven soft consolidation** that fires when a prompt exceeds the context budget. Each trigger archives at most one old prefix. Its boundary leaves a fixed recent suffix outside the new prefix (currently at least 8 messages) and extends backward so the retained turn starts with a `user` message. This policy has no configurable ratio. It only advances the internal `last_consolidated` cursor and leaves the session file untouched, so the raw tool-call trail stays on disk and can still be replayed or audited. If you rely on that trail for debugging or auditing, set `idleCompactAfterMinutes` to `0` and let only the token-driven path run.
