...
# ---------------------------------------------------------------------------
# detect_relevant_skills — trigger + description matching
# ---------------------------------------------------------------------------


class TestDetectRelevantSkills:
    """Test trigger-based and description-based skill detection."""

    def test_trigger_match_by_name(self, tmp_path: Path) -> None:
        _make_skill(tmp_path / "skills", "weather")
        loader = SkillsLoader(tmp_path)
        assert "weather" in loader.detect_relevant_skills("what is the weather?")

    def test_trigger_match_hyphenated_name(self, tmp_path: Path) -> None:
        _make_skill(tmp_path / "skills", "playwright-cli")
        loader = SkillsLoader(tmp_path)
        # "playwright cli" (without hyphen) is auto-generated trigger
        assert "playwright-cli" in loader.detect_relevant_skills("use playwright cli now")

    def test_no_match_returns_empty(self, tmp_path: Path) -> None:
        _make_skill(tmp_path / "skills", "weather")
        loader = SkillsLoader(tmp_path)
        assert loader.detect_relevant_skills("tell me a joke") == []

    def test_description_match(self, tmp_path: Path) -> None:
        """Skill matched via description keywords when triggers don't match."""
        _make_skill(
            tmp_path / "skills",
            "playwright-cli",
            skill_md=(
                ...
                "  screenshots, and data extraction. Navigate websites, interact\n"
                "  with web pages, take screenshots, test web applications.\n"
                "---\n# Playwright\n"
            ),
        )
        loader = SkillsLoader(tmp_path)
        # "playwright" alone doesn't match "playwright cli" trigger,
        # but "browser", "screenshots", "websites" match the description
        result = loader.detect_relevant_skills("open a browser and take screenshots of websites")
        assert "playwright-cli" in result

    def test_description_needs_multiple_hits(self, tmp_path: Path) -> None:
        """A single keyword overlap shouldn't trigger description matching."""
        _make_skill(
            tmp_path / "skills",
            "web-tool",
            skill_md=(
                "---\nname: web-tool\n"
                "description: >-\n"
                "  Browser automation screenshots testing forms navigation.\n"
                "---\n# Web\n"
            ),
        )
        loader = SkillsLoader(tmp_path)
        # Only one keyword hit ("browser") — should NOT activate
        result = loader.detect_relevant_skills("open a browser")
        assert "web-tool" not in result

    def test_triggers_take_priority(self, tmp_path: Path) -> None:
        """Skills matched by triggers appear before description-matched ones."""
        _make_skill(
            tmp_path / "skills",
            "alpha",
            skill_md=(
                "---\nname: alpha\ntriggers: [greet]\n"
                "description: greeting salutation hello world\n---\n# Alpha\n"
            ),
        )
        _make_skill(
            tmp_path / "skills",
            ...
            skill_md=(
                "---\nname: beta\n"
                "description: greeting salutation hello world welcome\n---\n# Beta\n"
            ),
        )
        loader = SkillsLoader(tmp_path)
        result = loader.detect_relevant_skills("greet hello world welcome")
        assert result[0] == "alpha"  # trigger match
        assert "beta" in result  # description match

    def test_max_skills_respected(self, tmp_path: Path) -> None:
        for i in range(6):
            _make_skill(
                tmp_path / "skills",
                f"skill-{i}",
        ...
        loader = SkillsLoader(tmp_path)
        result = loader.detect_relevant_skills("magic", max_skills=3)
        assert len(result) <= 3


class TestDescriptionKeywords:
    """Test the _description_keywords helper."""

    def test_filters_stopwords(self) -> None:
        kws = SkillsLoader._description_keywords("Use this for the web and browser automation")
        assert "web" in kws
        assert "browser" in kws
    ...
    def test_short_words_excluded(self) -> None:
        kws = SkillsLoader._description_keywords("AI is ok to go")
        # "ai", "is", "ok", "to", "go" — all <=2 chars
        assert kws == []

    def test_empty_description(self) -> None:
        assert SkillsLoader._description_keywords("") == []
