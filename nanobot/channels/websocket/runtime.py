...
class WebSocketChannel(BaseChannel):
    ...
    def _cleanup_connection(self, connection: ServerConnection) -> None:
        ...
        for cid in chat_ids:
            if _is_temporary_chat_id(cid) and self.gateway.session_manager is not None:
                self.gateway.session_manager.invalidate(f"{self.name}:{cid}")
        ...
        if t == "message":
            ...
            if temporary:
                ...
                if self.gateway.session_manager is None:
                    ...
                    return
                self.gateway.session_manager.get_or_create_memory_only(f"{self.name}:{cid}")
