...
class MessageTool(Tool, ContextAware):
    ...
    def set_suppress_delivery(self, active: bool):
        """Temporarily prevent proactive channel sends for an internal run."""
        return self._suppress_delivery_var.set(active)

    def reset_suppress_delivery(self, token) -> None:
        """Restore previous proactive delivery suppression state."""
    ...
    async def execute(
        ...
        **kwargs: Any,
        ...
        if self._suppress_delivery_var.get():
            return (
                "Message delivery suppressed for this internal run. "
                "Return any user-facing result as your final answer instead."
            )
