import { ArrowRightLeft, KeyRound, X } from "lucide-react";
import { useTranslation } from "react-i18next";

import { Button } from "@/components/ui/button";

/** A live fallback is informative, not a failed message or a changed preset. */
export function ModelFallbackNotice({ model, reauthProviderLabel, onDismiss, onOpenSettings }: {
  model: string;
  ...
  return (
    ...
    >
      {reauthProviderLabel
        ? <KeyRound className="mt-0.5 h-3.5 w-3.5 shrink-0 text-amber-600 dark:text-amber-400" aria-hidden />
