...
from janniebot.command.builtin import build_help_text
from janniebot.config.paths import get_media_dir
from janniebot.config.schema import Base
from janniebot.security.network import validate_url_target
from janniebot.utils.helpers import split_message
...
class TelegramChannel(BaseChannel):
    ...
    def _normalize_telegram_command(content: str) -> str:
        """Map Telegram-safe command aliases back to canonical janniebot commands."""
    ...
    async def start(self) -> None:
        ...
        self._app = tg_app
        tg_app.add_error_handler(self._on_error)

        # Add command handlers (using Regex to support @username suffixes before bot initialization)
        tg_app.add_handler(MessageHandler(filters.Regex(r"^/start(?:@\w+)?$"), self._on_start))
        tg_app.add_handler(
            MessageHandler(
        ...
        tg_app.add_handler(MessageHandler(filters.Regex(r"^/help(?:@\w+)?$"), self._on_help))
        ...
        try:
            await tg_app.bot.set_my_commands(self.BOT_COMMANDS)
    ...
    async def stop(self) -> None:
        ...
        if self._app:
            ...
            stop_updater = self._app.updater
            assert stop_updater is not None
            await stop_updater.stop()
    ...
    async def _on_start(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        await update.message.reply_text(
            f"👋 Hi {user.first_name}! I'm janniebot.\n\n"
    ...
    async def _on_error(self, update: object, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        if err is None:
            return
        summary = self._format_telegram_error(err)
